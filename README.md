...
// 获取配置开关的值
var boolValue = configInfo?.GetBoolValue(true);
// var stringValue = configInfo?.GetStringValue("");
// var intValue = configInfo?.GetIntValue();
```
...
var boolValue = configInfo?.GetBoolValue(false);
```
    ...
    void Start()
    {
        ...
        AbcSDKSpace.ABetterChoiceAPI.Init(config, result =>
        {
            if (result.statusCode == AbcSDKSpace.StatusCode.Success)
            {
                ...
                var shouldShowBanner = experiment?.GetBoolValue("should_show_banner", true);
                if (shouldShowBanner)
                {
                    // 显示 Banner
                }
